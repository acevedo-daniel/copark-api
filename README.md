...
- **Validation-first contracts**: Zod schemas powering both runtime validation and OpenAPI generation
...
- **Security baseline**: Helmet, CORS policy by environment, body size limits
...
- `REDIS_URL` optional (enables shared rate-limit store across instances)
...
> API docs are available in development mode only.
...
- Errors flow through a global handler with unified contract
