...
It can be bit tricky to achieve this in your codebase, because _require_ is synchronous and _import_ is a async.
...
```ts
const schema = z.string().openapi('Schema', { description: 'Name of the user', example: 'Test' });
...
```ts
const registry = new OpenAPIRegistry();
const schema = registry.register('Object', z.object({ key: z.string().openapi('Test') }));
```
